...
public class DP28LC516LongestPalindromicSubsequenceInString {
    ...
    public static void main(String[] args) {
        ...
        //Recursion
        System.out.println(recursion(s, sReverse, index1-1, index2-1));

        //Memoization
        int[][] dp=new int[index1+1][index2+1];
        for(int[] r:dp)
            Arrays.fill(r,-1);
        System.out.println(memoization(s, sReverse, index1-1, index2-1, dp));

        //Tabulation
        int[][] dp1=new int[index1+1][index2+1];
        System.out.println(tabulation(s,sReverse,index1,index2,dp1));

        System.out.println("Verify the Longest Palindrome");
        int i=index1;
        int j=index2;
