...
public class G52LC827MakingLargeIsland {
    ...
    private static int makingLargeIsland(int[][] grid) {
        ...
        return max;
        //Time Complexity: O(N^2*4)+O(N^2*4) ~ O(N2) where N = total number of rows of the grid. Inside those nested loops, all the operations are taking apparently constant time. So, O(N2) for the nested loop only, is the time complexity.
