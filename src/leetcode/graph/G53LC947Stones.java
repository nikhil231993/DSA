...
public class G53LC947Stones {
    ...
    private static int stonesRemoved(int[][] stones) {
        ...
        int maxCol=0;
        for(int[] r:stones){
            maxRow=Math.max(maxRow, r[0]);
            maxCol=Math.max(maxCol, r[1]);
        }

        //Do the union considering columns as an extension of row
        DisjointSetBySize ds=new DisjointSetBySize(maxRow+maxCol+1);//Since Disjointset class handles n+1 so we give only +1

        //Map to store only row and columns having stones so that it can be used in findParent function
        Map<Integer,Integer> map=new HashMap<>();
        for(int[] r: stones){
            int row=r[0];
            int col=r[1]+maxRow+1;
