...
public class G47KrushkalAlgorithm {
    ...
    public static void main(String[] args) {
        ...
        List<List<Pair47>> adjList=new ArrayList<>();
        createAdjacencyList(V,edges,adjList);
        ...
        for(int i=0;i<adjList.size();i++){
            for(int j=0;j<adjList.get(i).size();j++){
                Pair47 pair=adjList.get(i).get(j);
                int wt=pair.dist;
                int u=i;
                int v=pair.node;
                Edge edge=new Edge(wt,u,v);
                edgeList.add(edge);
            }
        }
...
//Space Complexity: O(N) + O(N) + O(E) where E = no. of edges and N = no. of nodes. O(E) space is taken by the array that we are using to store the edge information. And in the disjoint set data structure, we are using two N-sized arrays i.e. a parent and a size array (as we are using unionBySize() function otherwise, a rank array of the same size if unionByRank() is used) which result in the first two terms O(N).
    ...
    private static void createAdjacencyList(int v, int[][] edges,List<List<Pair47>> adjList) {

        for(int i=0;i<v;i++)
            adjList.add(new ArrayList<>());

        for(int i=0;i< edges.length;i++){
            adjList.get(edges[i][0]).add(new Pair47(edges[i][1],edges[i][2]));
            adjList.get(edges[i][1]).add(new Pair47(edges[i][0],edges[i][2]));
        }
    }
}
