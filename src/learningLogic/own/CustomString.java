...
public class CustomString {
	...
	public String remove(String arg) {
		...
		if(!this.myString.trim().isEmpty() || !isSet || this.myString == null)
		{
			return "";
		}
		
		if (arg.matches("[^a-zA-Z]+") || arg.trim().isEmpty()) {
			  return this.myString;
			}
		
		char[] args=arg.toCharArray();
		if(this.myString != null && !this.myString.trim().isEmpty() && isSet !=false) {	
			
			for(int i=0;i<args.length;i++) {
				 strNew = this.myString.replace(String.valueOf(arg.charAt(i)), "");
				 this.myString=strNew;
			}
		}
